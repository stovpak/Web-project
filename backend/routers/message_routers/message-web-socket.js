  ...
  ws.on("message", message => {
    ...
    if (messageType === "Connect") {
      ...
      switch (messageType) {
        ...
        case "Update":
          messageService
            .findMessage(JSON.parse(message).messageId)
            .then(mgs => {
              if (
                authorValidator(login, mgs.author_name) &&
                adminValidator(role)
              ) {
                messageService
                  .updateMessage(
                    JSON.parse(message).messageId,
                    JSON.parse(message).text
                  )
                  .then(responseMessage => {
        ...
        case "Like":
          ...
          topicLikeService
            .countLikes(JSON.parse(message).topicId)
            .then(topic => {
